...
class TimetableScreenTest {
    ...
    private fun fullState(
        ...
        semester = SemesterDefaults.AUTUMN_2026(id = "s", profileId = "p"),
        viewedWeek = 2,
    ...
    @Test fun screen_renders_semester_week_and_date_range() {
        rule.setContent { TimetableScreen(state = fullState(), onPrevWeek = {}, onNextWeek = {}) }
    ...
    @Test fun screen_renders_weekly_grid() {
        rule.setContent { TimetableScreen(state = fullState(placedSchool = placedSchool()), onPrevWeek = {}, onNextWeek = {}) }
    ...
    @Test fun arrows_fire_prev_next_callbacks() {
        ...
        rule.setContent { TimetableScreen(state = fullState(), onPrevWeek = { prev++ }, onNextWeek = { next++ }) }
    ...
    @Test fun manual_academic_current_hides_refresh() {
        ...
        rule.setContent { TimetableScreen(state = fullState(isAcademicCurrentViewed = true, canSyncViewed = false), onPrevWeek = {}, onNextWeek = {}) }
        rule.onAllNodesWithTag("refresh").assertCountEquals(0)
    }

    @Test fun portal_current_shows_disabled_refresh() {
        rule.setContent { TimetableScreen(state = fullState(isAcademicCurrentViewed = true, canSyncViewed = true), onPrevWeek = {}, onNextWeek = {}) }
    ...
    @Test fun empty_state_has_no_dashboard() {
        rule.setContent { TimetableScreen(state = TimetableUiState(isLoading = false), onPrevWeek = {}, onNextWeek = {}) }
